...
private:
	...
	static float*generateMatrix(int N, int M)
	{
		...
		std::generate(result, result + N * M, []() {return distrib(device) % 2 == 0 ? -1 : 1; });
		return result;
	}
	...
	static void printMtrix(float*matrix, int N, int M) {
		for (int i = 0; i < N;i++)
		{
			for (int j = 0; j < M;j++) {
	...
	static void printMtrixTranspose(float*matrix, int N, int M) {
		for (int j = 0; j < M;j++)
		{
			for (int i = 0; i < N;i++) {
				std::cout << matrix[i*M + j] << " ";
...
public:
	static void Main() {
		...
		if (process.IsMaster()) {
			...
			printMtrixTranspose(firstMatrix, dimL, dimk);
			std::cout << "The second matrix: \n";
			printMtrix(secondMatrix, dimL, dimM);
