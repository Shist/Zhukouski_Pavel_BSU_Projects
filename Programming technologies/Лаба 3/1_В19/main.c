...
void Previous_Day(int g, int m, int n)
{
	...
	if (n == 0)
	{
		...
		if (m == 4 || m == 6 || m == 9 || m == 11) // јпрель, июнь, сент€брь, но€брь - 30 дней
			n = 30;
		else if (m == 2) // ‘евраль - 28/29 дней
		{
			if (g % 4 == 0) // ѕроверка года на високосность
				n = 29;
			else
				n = 28;
		}
		else // ќстальные мес€цы - 31 день
			n = 31;
	}
